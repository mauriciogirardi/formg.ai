...
export function SelectPropertiesComponent({
  ...
  parentId,
  positionIndex,
}: SelectPropertiesComponentProps) {
  const block = blockInstance as NewInstance
  const onUpdateChildBlock = useBuilderStore(store => store.onUpdateChildBlock)

  const { helperText, label, options, placeHolder, required } = block.attributes
  ...
  }, [form, label, helperText, required, placeHolder, options])
  ...
  return (
    <div className="w-full pb-4">
      <PropertyName name={`Select ${positionIndex}`} />
